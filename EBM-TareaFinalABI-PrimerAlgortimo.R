...
individuo<-0
individuo[1]<-list(creacionloteinicial[1:30])
individuo[2]<-list(creacionloteinicial[31:60])
individuo[3]<-list(creacionloteinicial[61:90])
...
pooluntakenfish<-setdiff(pooluntakenfish,individuo[[1]])
pooluntakenfish<-setdiff(pooluntakenfish,individuo[[2]])
pooluntakenfish<-setdiff(pooluntakenfish,individuo[[3]])
...
selecrepro<-function(){
  ...
  for (j in 1:nIter){
    ...
    #individuo<-reproduccion(individuo)
    individuo<-reproduccionmejor(individuo)
    
    ##Mutacion de los lotes, quitando los que mas aportan al lote. 
    
    #mutlote<-mutacionsimple(individuo,j,pooluntakenfish)
    #mutlote<-mutacionsimplemejor(individuo,j,pooluntakenfish)
    #mutlote<-mutacionsimplelotemedia(individuo,j,pooluntakenfish)
    mutlote<-mutacionsimplelotemax(individuo,j,pooluntakenfish)
    
    ##Actualizacion de los lotes de peces y de los peces que quedan fuera
    individuo<-mutlote[[1]]
    pooluntakenfish<-mutlote[[2]]
    
    ##cat(j, "Máximo individuo 1: ",evaluacionmaximolote(individuo[[1]])[[1]],"  Máximo individuo 2: ",evaluacionmaximolote(individuo[[2]])[[1]],"  Máximo individuo 3: ",evaluacionmaximolote(individuo[[3]])[[1]],"\n")
    ##cat("   Media individuo 1: ",evaluacionmedialote(individuo[[1]]),"  Media individuo 2: ",evaluacionmedialote(individuo[[2]]),"  Media individuo 3: ",evaluacionmedialote(individuo[[3]]),"\n")
    
    ## Dibujo
    media1  <- c (media1,  evaluacionmedialote(individuo[[1]]))
    media2  <- c (media2,  evaluacionmedialote(individuo[[2]]))
    media3  <- c (media3,  evaluacionmedialote(individuo[[3]]))
    maxim1 <-c (maxim1,  evaluacionmaximolote(individuo[[1]])[[1]])
    maxim2 <-c (maxim2,  evaluacionmaximolote(individuo[[2]])[[1]])
    maxim3 <-c (maxim3,  evaluacionmaximolote(individuo[[3]])[[1]])
    ...
    vmaxim<-c(evaluacionmaximolote(individuo[[1]])[[1]],evaluacionmaximolote(individuo[[2]])[[1]],evaluacionmaximolote(individuo[[3]])[[1]])
    if(max(vmaxim)<maximminimal){
      lotemaximinimo<-individuo
      maximminimal<-max(vmaxim)
    }
    maximminima<-c(maximminima,maximminimal)
    lines(maximminima,col=4,type = "l",lty="solid")
    
    ##Calculo de la media del individuo y dibujo del mismo
    vmedia<-c(evaluacionmedialote(individuo[[1]]),evaluacionmedialote(individuo[[2]]),evaluacionmedialote(individuo[[3]]))
...
reproduccionmejor<-function(lote){
  ...
  padres<-c(1:3)
  loteminimo<-1
  maxlotes<-lapply(lote, maximorelatednessindividuoini)
  loteminimo<-which.min(unlist(maxlotes))
  for (i in 1:nLotes){
    ##cat("Lote: ",i,"\n")
    maximolotes[i]<-evaluacionmaximolote(lote[[i]])[[1]]
    ##cat("Para seleccion de padres: \nMedia lote ",i,": ",evaluacionmedialote(lote[[i]]),"\n")
    if(maximolotes[i]==min(maximolotes)){
      loteminimo<-i
      ##cat(loteminimo,"\n")
    }
  }
